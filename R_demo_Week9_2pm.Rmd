...
penguins_clean <- penguins %>% filter(!is.na(bill_length_mm) & !is.na(bill_depth_mm))
...
# And now let's add the fitted regression line to our plot
penguins_clean %>% ggplot(aes(x = bill_depth_mm, y = bill_length_mm)) +
  geom_point() +
...
# Let's fit this new model with two predictors
model2 <- lm(bill_length_mm ~ bill_depth_mm + species, data=penguins_clean)
summary(model2)$coefficients


# What do the last two beta-hat estimates represent?
# They are the predictors for the species predictor
# Baseline level: Adelie because it doesn't show up in the table
# x2 = I(species is Chinstrap)
# x3 = I(species is Gentoo)
# x2 = 0, x3 = 0: Adelie
# x2 = 0, x3 = 1: Gentoo
# x2 = 1, x3 = 0: Chinstrap
# x2 = 1, x3 = 1: impossible

# Fitted regression eq: y-hat = beta0-hat + beta1-hat*billdepth + beta2-hat*I(species is Chinstrap) + beta3hat*I(species is Gentoo)


# And add the fitted line(s) to the scatterplot
library(broom)
penguins_clean %>% ggplot(aes(x = bill_depth_mm, y = bill_length_mm, color = species)) +
  geom_point() +
...
# Let's add the fitted line to the plot
penguins_clean %>% ggplot(aes(x = bill_depth_mm, y = bill_length_mm, color = species)) +
  geom_point() +
  geom_smooth(method="lm", se=FALSE)
